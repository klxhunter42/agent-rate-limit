...
+---+------+  +------+-------+
   ...
   +---------+--------+
             ...
             +-----------> Lotus LLM (api-cpxis.lotuss.com/llm)
...
| Secrets mounted | `ZAI_API_KEYS`, `LOTUS_API_KEYS`, `GEMINI_OAUTH_CLIENT_ID`, `GEMINI_OAUTH_CLIENT_SECRET` |
...
| Secrets mounted | `GLM_API_KEYS`, `OPENAI_API_KEYS`, `ANTHROPIC_API_KEYS`, `GEMINI_API_KEYS`, `OPENROUTER_API_KEYS`, `LOTUS_API_KEYS` |
...
| `lotus-api-keys` | `secrets.lotusApiKeys` |
...
- Forwards to Lotus upstream (`api-cpxis.lotuss.com/llm/v1/chat/completions`)
- Converts response back to Anthropic format
- Non-streaming only
- Health check: `GET /health`

### 7.2 `lotus-sse-patch-proxy.py`

Streaming SSE event patcher for Lotus upstream.

- Listens on port 8999
- Proxies `/v1/messages` to Lotus upstream
...
| `LOTUS_API_KEYS` | Lotus LLM API keys |
