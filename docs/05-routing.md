...
|   |   map model to target's default model (e.g. claude -> lotus -> "default")
...
|   |-- Provider-level model override (e.g. lotus -> "default")
|   |-- Provider-level max_tokens clamp (e.g. lotus -> 4096)
...
| `target` | Yes | Provider ID (e.g., `claude-oauth`, `gemini-oauth`, `anthropic`, `lotus`) |
...
| `lotus` | openai | bearer | `/v1/chat/completions` | Lotus, model override "default", max_tokens 4096, tool_mode "native", 3 continuations |
